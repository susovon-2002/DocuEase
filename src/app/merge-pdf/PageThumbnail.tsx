...
export function PageThumbnail({ thumbnailUrl, pageNumber, fileName }: PageThumbnailProps) {
  return (
    <div className="relative rounded-md shadow-md cursor-grab bg-white p-1 border">
      <img
        src={thumbnailUrl}
        alt={`Page ${pageNumber}`}
      ...
      {fileName && (
        <Badge variant="outline" className="absolute top-2 left-2 max-w-[calc(100%-1rem)] truncate">
          {fileName}
        </Badge>
      )}
    </div>
