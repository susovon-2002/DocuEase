...
import { Loader2, UploadCloud, Download, RefreshCw, Image as ImageIcon } from 'lucide-react';
...
export function PdfToJpgClient() {
  ...
  const handleConvert = async (file: File) => {
    ...
    setProcessingMessage('Converting PDF to JPGs...');
    ...
    try {
        ...
        setStep('download');
        toast({ title: 'Conversion Complete!', description: 'Your JPG images are ready.' });
  ...
  const handleDownloadAllAsZip = async () => {
    ...
    try {
        ...
        for (let i = 0; i < outputImages.length; i++) {
            ...
            zip.file(`${originalFile?.name.replace('.pdf','') || 'page'}_${i + 1}.jpg`, blob);
        }

        const zipBlob = await zip.generateAsync({ type: 'blob' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(zipBlob);
        link.download = `${originalFile?.name.replace('.pdf','')}__images.zip`;
      ...
      return (
        <div className="w-full max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground mt-2">Convert each page of your PDF into high-quality JPG images.</p>
          </div>
        ...
        return (
            <div className="w-full max-w-4xl mx-auto text-center">
                 <div className="mb-8">
                    ...
                    <p className="text-muted-foreground mt-2">Your images are ready to be downloaded as a ZIP file.</p>
                </div>
                <Card className="mb-8">
                    <CardHeader>
                        <CardTitle>Image Preview</CardTitle>
                        <CardDescription>{outputImages.length} images generated.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-96 overflow-y-auto">
                        {outputImages.map((src, index) => (
                            <div key={index} className="relative">
                                <img src={src} alt={`Page ${index + 1}`} className="rounded-md w-full aspect-[2/3] object-contain bg-muted" />
